...
<script>
// ── THEME ──
(function(){const r=document.documentElement,btn=document.getElementById('themeBtn');let d='dark';r.setAttribute('data-theme',d);btn.addEventListener('click',()=>{d=d==='dark'?'light':'dark';r.setAttribute('data-theme',d);});})();
...
function renderTaskCard(t){
  ...
  const resp=t.result?.response||t.error||'';
  const isError=status==='failed'||resp.toLowerCase().startsWith('[ollama error]');
  ...
  const ts=(t.completed_at||t.created_at||'').slice(0,19).replace('T',' ');
...
function toggleTaskBody(id){const el=document.getElementById(id);if(el)el.classList.toggle('open');}
...
async function createAndAssign(){
  ...
  await fetch('/task/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task_id:id,prompt,model})});
...
async function refreshLogs(){
  ...
  try{const logs=await(await fetch(url)).json();renderLogs(logs);}catch(e){}
}
...
// ── DIAGNOSTICS ──
async function diagMeshNodes(){document.getElementById('dMesh').textContent='...';const d=await(await fetch('/mesh/nodes')).json();document.getElementById('dMesh').textContent=JSON.stringify(d,null,2);}

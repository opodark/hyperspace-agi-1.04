...
def create_task():
    ...
    push_log('system', f'Task created: {task_id}', detail=f'prompt={prompt[:80]}', status='info')
    ...
    <div class="diag-grid">
      ...
      <div class="card">
        ...
        <div style="display:flex;gap:7px;flex-wrap:wrap;margin-bottom:8px">
          <input id="drNode" class="inp" placeholder="node-id" style="width:120px"/>
  ...
  <div id="panel-setup" class="panel">
    ...
    <div class="card">
      <div class="card-title">
        ...
        <span id="rsAt" style="margin-left:auto;font-size:.65rem;color:var(--text-muted)"></span>
      </div>
      <div class="fg">
        <label class="label">Shared Secret</label>
        <div class="secret-row">
          <input id="secVal" type="password" class="inp" placeholder="Leave blank to keep current"/>
          <button class="btn btn-ghost btn-sm" onclick="toggleSec()">Show</button>
    ...
    <div class="card">
      ...
      <div class="setup-grid">
        <div class="fg">
          ...
          <input id="aUrl" class="inp" placeholder="http://authority:8080"/>
          <span class="hint">Endpoint REST dell'authority.</span>
        </div>
        <div class="fg">
          ...
          <span class="hint">Modalità autenticazione.</span>
        </div>
        <div class="fg">
          <label class="label">Status</label>
          <div class="mode-row" style="max-width:220px">
    ...
    <div class="card">
      ...
      <div class="setup-grid">
        <div class="fg">
          ...
          <span class="hint">Endpoint del servizio Ollama locale.</span>
        </div>
        <div class="fg">
          <label class="label">Default Model</label>
          <input id="oModel" class="inp inp-mono" placeholder="phi3"/>
          <span class="hint">Modello Ollama di default per i task.</span>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="setup-grid">
        <div class="fg" style="grid-column:span 2">
          ...
          <div class="mode-row" style="max-width:340px">
          ...
          <span class="hint">Pure Mesh attiva coordinamento P2P con Modular Hash Tree.</span>
        </div>
        ...
        <div class="fg mesh-section" id="meshMhtSection">
          ...
          <span class="hint">Abilita Modular Hash Tree per mesh routing avanzato.</span>
        </div>
      </div>
    </div>
...
function renderLogs(logs){
  ...
  body.innerHTML=logs.map((l,i)=>`
    <div class="log-row" onclick="toggleD('ld${i}')">
      ...
      <span><span class="tbadge tb-${l.type}">${l.type.replace(/_/g,'&nbsp;')}</span></span>
      <span class="nc">${escH(l.sourceNode||'—')}</span>
      <span class="nc">${escH(l.targetNode||'—')}</span>
      <span><span class="sbadge st-${l.status}">${stEmoji[l.status]||''}&nbsp;${l.status}</span></span>
      <span class="summary">${escH(l.summary)}</span>
    </div>
    <div class="detail-row" id="ld${i}">
<b>TraceID:</b> ${escH(l.traceId)} &nbsp;|&nbsp; <b>Detail:</b>\n${escH(l.detail||'—')}
    </div>
  `).join('');
  if(document.getElementById('autoScroll').checked){
    const lb=body; lb.scrollTop=lb.scrollHeight;
  }
}
...
async function sendDream(){
  ...
  const r=await fetch('/logs/add',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({type:'dream',summary:sum,sourceNode:node,status:'info',detail:'Injected via Diagnostics'})});
  ...
  try{
    ...
    await Promise.all(list.map(async n=>{
      ...
      try{
        ...
        results[n.node_id]={status:'ok',latency:(Date.now()-start)+'ms'};
        await fetch('/logs/add',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({type:'connection_test',summary:`Ping OK → ${n.node_id}`,detail:`latency: ${Date.now()-start}ms`,sourceNode:'control-plane',targetNode:n.node_id,status:'success'})});
      }catch(e){
...
async function checkOllamaModal(){
  ...
  if(d.ok){
    ...
    if(!d.models||!d.models.length){
      list.innerHTML='<div style="color:var(--text-muted);font-size:.72rem;padding:8px">No models found. Run: <code>docker exec hyperspace_ollama ollama pull phi3</code></div>';
    }else{
    ...
    dot.classList.add('err');dot.classList.remove('ok');
    list.innerHTML='';
    err.textContent='Error: '+(d.error||'unreachable');
...
async function loadCfg(){
  ...
  document.getElementById('rsAt').textContent=c.security.secretRotatedAt?'rotated '+c.security.secretRotatedAt.slice(0,10):'never rotated';
  document.getElementById('aUrl').value=c.authority.serverUrl||'';
  document.getElementById('aMode').value=c.authority.authMode||'none';
  setAuthEnabled(c.authority.enabled);
...
async function saveCfg(){
  ...
  const payload={
    ...
    authority:{serverUrl:document.getElementById('aUrl').value,authMode:document.getElementById('aMode').value,enabled:_authEnabled},

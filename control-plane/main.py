...
def _ranked_executable_nodes(active_nodes: list) -> list:
    ...
    il nodo successivo se il migliore fallisce (es. non ha il modello
    richiesto). _node_score non conosce i modelli disponibili su un nodo, solo
    tier/vram/peers/uptime: senza questo fallback, un nodo con score piu' alto
    ma privo del modello richiesto blocca per sempre l'accesso agli altri nodi
    della mesh."""
    executable = [n for n in active_nodes if _best_endpoint(n)]
...
def v1_chat_completions():
    ...
    for candidate in _ranked_executable_nodes(active):
